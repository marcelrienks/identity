...
detect_file_changes() {
    ...
    # Save change manifest for next step
    local change_manifest=".deploy/changes.json"
    cat > "$change_manifest" << EOF
{
  "timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "added": [$(IFS=','; echo "${added_files[@]/#/\"}" | sed 's/,/\",\"/g')],
  "modified": [$(IFS=','; echo "${modified_files[@]/#/\"}" | sed 's/,/\",\"/g')],
  "deleted": [$(IFS=','; echo "${deleted_files[@]/#/\"}" | sed 's/,/\",\"/g')]
}
EOF
    ...
    for file in "${files_to_upload[@]}"; do
        ...
        (
            ...
            while [[ $retry -lt $max_retries ]]; do
                if s3_upload_object "$s3_bucket" "$file" "$local_path" \
